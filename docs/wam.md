...
为了证明未来分支确实在起作用，至少可以比较：

| 对照                  | 保留内容                         | 需要回答的问题                    |
| --------------------- | -------------------------------- | --------------------------------- |
| policy-only           | 只有动作策略                     | WAM 相对普通 VLA 的净收益是多少？ |
| future auxiliary      | 训练时有未来损失，推理不 rollout | 未来监督是否改善动作表示？        |
| action-conditioned WM | 未来分支接收候选动作             | 未来是否真正对动作敏感？          |
| no-action future      | 去掉动作条件                     | 只预测常见未来是否已经足够？      |
| shuffled-action       | 打乱动作与后果的对应关系         | 模型是否依赖真实因果对齐？        |
| explicit rollout      | 推理时对候选动作 rollout         | 想象收益是否抵消延迟和模型误差？  |

每个对照都应使用相同数据、动作格式、控制器、chunk 设置和随机种子。只比较未来视频指标，无法替代成功率、动作排序准确率、失败恢复和安全事件。
...
建议再记录一条端到端链路：观测采集 $\rightarrow$ 编码 $\rightarrow$ 候选动作采样 $\rightarrow$ future rollout $\rightarrow$ 评分 $\rightarrow$ 安全过滤 $\rightarrow$ 下发 $\rightarrow$ 实际接管。对每一段报告平均值和高分位延迟，否则 WAM 在离线评测中有效、部署时却来不及接管的情况很容易被忽略。

还要特别看 attention mask 和 teacher forcing：动作 token 是否偷看真实未来，训练时使用的未来状态是否在部署时不可获得，以及 inverse dynamics 是否依赖未来观测。任何部署时拿不到的变量都不能作为推理输入。
