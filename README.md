...
具身智能学习地图，面向想从机器人基础、具身数据走到 VLA、World Model、RL/MBRL、WAM 和 Robot Agent 的读者。

内容从坐标系、运动学、动力学、ROS 2、仿真和控制流程开始，逐步连接到具身数据采集与处理、视觉语言动作策略、世界模型、模型辅助强化学习、世界动作模型，以及负责任务编排、工具调用、记忆和失败恢复的 Robot Agent。每条路线都配有论文、开源代码、benchmark 与实践入口，并把 observation、action、时间对齐、闭环评测和真机部署放在同一条链路里。
...
| 📦 Embodied Data | 想采集、处理和混合训练数据     | [具身数据专题](docs/embodied-data.md) |
...
| 🧠 Robot Agent | 想做长时程规划、工具、记忆和恢复 | [Robot Agent 专题](docs/robot-agent.md) |
...
| 构建 Robot Agent         | [Robot Agent 专题](docs/robot-agent.md) | RPent、Harness VLA、工具调用、记忆与恢复       |
...
flowchart LR
    ...
    WAM --> A
    MBRL --> A
    A --> AGENT["Robot Agent：编排、验证与恢复"]
    AGENT --> C
    E -->|"反馈"| O
```

- **具身数据**规定观测、动作、坐标系、时间同步和任务标签；**VLA** 直接学习从视觉和语言到动作；**WM** 预测动作条件下的未来状态；**WAM** 把未来预测结构性接入动作生成；**Robot Agent** 在这些模型和解析技能之上负责任务编排、验证、记忆与恢复。
...
- [知识图谱](docs/knowledge-map.md)：统一说明具身数据、VLA、WM、WAM、MBRL、RL 和 Robot Agent 的关系。
...
- [Robot Agent](docs/robot-agent.md)：RPent、Harness VLA、工具注册、执行验证、记忆、恢复、主动感知与实时安全。
- [论文清单](docs/papers.md)：按具身数据、VLA、WM、WAM、Robot Agent、MBRL、RL 和 benchmark 分级阅读。
...
- [学习路线](docs/roadmap.md)：按 10 章组织阅读和实践顺序，新增具身数据与 Robot Agent。
- [术语表](docs/glossary.md)：缩写、数据/Agent 术语、相近概念和容易混淆的边界。
...
- **具身数据与 Robot Agent**：[Qwen-RobotManip](https://github.com/QwenLM/Qwen-RobotManip)、[Universal Manipulation Interface](https://github.com/real-stanford/universal_manipulation_interface)、[Open-TeleVision](https://github.com/OpenTeleVision/TeleVision)、[DROID](https://github.com/droid-dataset/droid)、[RPent](https://github.com/RLinf/RPent) 和 [Harness VLA](https://harnessvla.github.io/)。这些项目提供跨本体对齐、遥操作采集、数据组织、任务编排、记忆和失败恢复的公开参考。
